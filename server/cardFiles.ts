...
type CardMedia = { avatarUrl?: string | null; coverUrl?: string | null; logoUrl?: string | null; portfolio?: string | null };
...
export function ownedUploadKeys(card: CardMedia, ownerUserId: number): Set<string> {
  ...
  const text = [card.avatarUrl, card.coverUrl, card.logoUrl, card.portfolio].filter(Boolean).join("\n");
